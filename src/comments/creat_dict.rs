...
use crate::{
    ...
    comments::dict::{creat_cmt, all_cmt, id_cmt, update_cmt, del_cmt},
...
pub async fn get_creat_cmt(
    ...
    context.insert("t", &t);
    context.insert("dict", &result);
    Ok(Html(templates.render("dict", &context).unwrap()))
}
...
pub async fn get_update_cmt(
    ...
    let t = match i.ctx(headers).await {
        ...
        Ok(None) | Err(None) => {
            ...
            return Err(
                Html(templates.render("update_dict", &context).unwrap())
            )
        }
    };
    let cmt = id_cmt(
    ...
    Ok(Html(templates.render("update_dict", &context).unwrap()))
...
pub async fn post_update_cmt(
    ...
    Ok(
        Redirect::to(&("/creat-dict/".to_owned() + &to_id.to_string()))
        .into_response()
    )
}
